...
cmd_export(void)
{
    ...
    /* ask user for the filename */
    printf("Please enter template file name (use underscore as spaces): ");
    if (scanf("%s", templateFileName) != 1) {
        printf("Cannot get the file name.\n");
        return;
    }
    // add .fing to end of file name
	strcat(templateFileName, ".fing");
	char *path = "./Enrolling/";
	char *directory = malloc(strlen(path)+1+4);
	strcpy(directory, path);
	strcat(directory, templateFileName);
	//char *directory = "./Enrolling/";
	//char *file_name = strcat(directory, templateFileName);
	//printf(file_name);
...
main(int argc, char** argv)
{
    ...
    printf("Enroll the finger you want to add...\n");
    ABSEnroll(conn, &op, &tset[0], 0);
    cmd_export();
